...
if __name__ == '__main__':
    split_rate = 0.3    # 训练集和验证集划分比率
    resize_image = 224    # 图片缩放后统一大小
    file_path = 'D:\datasets\Meander'    # 获取原始数据集路径
    ...
    for path in dirs:
        ...
        os.makedirs("D:\\datasets\\Meander\\train\\{}".format(path), exist_ok=True)
        os.makedirs("D:\\datasets\\Meander\\test\\{}".format(path), exist_ok=True)
        ...
        for i, file in enumerate(files):
            ...
            if i < split_boundary:
                new_im.save(os.path.join("D:\\datasets\\Meander\\test\\{}".format(path),
                                         file.split('\\')[-1].split('.')[0] + '.jpg'))
            else:
                new_im.save(os.path.join("D:\\datasets\\Meander\\train\\{}".format(path),
                                         file.split('\\')[-1].split('.')[0] + '.jpg'))
